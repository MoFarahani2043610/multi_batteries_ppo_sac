"""
Richer observation experiment for N=1: adds two causal, price-history-based
features to the observation, on top of the existing SoC/price/time features:
...
steps, seed, evaluation procedure) is held identical to the N=1 baseline,
so any difference in performance can be attributed to the added features.
"""
import sys, os
...
base_features = make_time_features_2dim(len(prices))  # existing sin/cos time features, shape (T, 2)

# ── compute the two new causal features ─────────────────────────
price_flat = prices.flatten()
T = len(price_flat)

# Rolling mean of the past ROLLING_WINDOW steps (causal: uses [t-window, t])
...
# Price change over the past ROLLING_WINDOW steps (causal: p_t - p_{t-window})
dp = np.zeros(T, dtype=np.float32)
dp[ROLLING_WINDOW:] = price_flat[ROLLING_WINDOW:] - price_flat[:-ROLLING_WINDOW]
# first `ROLLING_WINDOW` steps have no valid lookback -> 0 (no change known yet)

# Normalize the new features to a similar scale as the existing normalized
# price feature (divide by price_ref, same convention as the price obs).
PRICE_REF = 52.0
p_bar_norm = (p_bar / PRICE_REF).astype(np.float32)
dp_norm = (dp / PRICE_REF).astype(np.float32)

# ── build richer feature matrix: [sin, cos, p_bar_norm, dp_norm] ──
...
# ── build env with richer features ───────────────────────────────
...
print(f"Observation shape with richer features: {obs.shape}  (expected (6,): SoC(1)+price(1)+sin+cos+p_bar+dp)")

# ── build config (identical to N=1 baseline except observation) ───
config = {
    ...
    "logging": {"log_dir": "m3_logs/sac_n1_richer_obs_test", "tensorboard": True,
                "save_freq": 100_000, "verbose": 1},
}

agent = SACAgent(env, config, seed=0)

# CRITICAL FIX: SACAgent._make_fresh_env() builds its own environment
# internally (used by SB3's EvalCallback during training) from the config
# dict alone, using data/loader.py's default 6-dim make_features() --
# NOT the richer 4-dim feature set (sin, cos, p_bar, dp) used in `env`
# above. Without this override, the periodic eval callback inside
# agent.train() would build a mismatched-shape observation space and
# crash. We patch it here so training-time evaluation uses the exact
# same richer features as training itself.
...
# ── evaluate on the SAME richer-feature environment (fresh episodes) ──
returns = []
for ep in range(10):
